...
enum class token_types //输入的字符串分为三种次元，数值型/运算符/其他
{
	null,
	number,
	signal
};
class token_base //次元基类
{
public:
	token_base() = default;
...
calculator::calculator(const std::string &str)
{
	...
	if (!m_tokens.empty())//check if token sequence is crossed by numbers and signals 
	{
		if (m_tokens.front()->get_type() == token_types::signal && static_cast<token_signal *>(m_tokens.front())->data == signal_types::sub_)
			m_tokens.push_front(new token_number("0"));//Only handle the first negative sign of a string
		status = token_types::number;
		for (auto &it : m_tokens)
		{
			if (it->get_type() == status)
			{
				...
				throw std::runtime_error("The next token isn't expected!");
		}
		if (status != token_types::signal)
			throw std::runtime_error("The last token is not number!");
	}
}
